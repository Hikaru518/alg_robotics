...
bool isValidStateWeird(const ompl::base::State* state, double sideLength, const std::vector<Rectangle>& obstacles)
{
    ...
    // Extract x, y, theta
    const ompl::base::RealVectorStateSpace::StateType* r1state = cstate->as<ompl::base::RealVectorStateSpace::StateType>(0);
    double r = r1state->values[0];

    const ompl::base::SO2StateSpace::StateType* so2State = cstate->as<ompl::base::SO2StateSpace::StateType>(1);
    double theta = so2State->value;

    double x = cos(theta) * r - 2;
    double y = sin(theta) * r - 2;
...
void planWithSimpleSetupWeird(const std::vector<Rectangle>& obstacles)
{
    ...
    ompl::base::StateSpacePtr r1(new ompl::base::RealVectorStateSpace(1));
    // We need to set bounds on R^2
    ompl::base::RealVectorBounds bounds(1);
    bounds.setLow(0); // x and y have a minimum of -2
    bounds.setHigh(4 * sqrt(2.0)); // x and y have a maximum of 2

    // Cast the r2 pointer to the derived type, then set the bounds
    r1->as<ompl::base::RealVectorStateSpace>()->setBounds(bounds);
    ...
    // Step 2) Create the SimpleSetup container for the motion planning problem.
    ompl::geometric::SimpleSetup ss(r1so2);

    // Step 3) Setup the StateValidityChecker
    ss.setStateValidityChecker(std::bind(isValidStateWeird, _1, 0.3, obstacles));

    // Step 4) Specify the start and goal states
    ompl::base::ScopedState<> start(r1so2);
    start[0] = 0.2;
    start[1] = M_PI/4;

    ompl::base::ScopedState<> goal(r1so2);
    goal[0] = 4 * sqrt(2.0) - 0.2;
    goal[1] = M_PI/4;
